...
r4 = s.get('https://jwxt.njfu.edu.cn/jsxsd/kbcx/kbxx_xqsj_ifr')
soup4 = BeautifulSoup(r4.text, 'html.parser')
print('jx0601 skyx options:', len(soup4.select('select[name=skyx] option')))
print('jx0601 xq options:', len(soup4.select('select[name=xq] option')))
print('jx0601 jxl options:', len(soup4.select('select[name=jxl] option')))

print(BeautifulSoup(s.get('https://jwxt.njfu.edu.cn/jsxsd/framework/xsrkxz.htmlx').text, 'html.parser').find_all('a'))
