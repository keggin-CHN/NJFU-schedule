...
r_form = s.get('https://jwxt.njfu.edu.cn/jsxsd/kbcx/kbxx_classroom')
soup = BeautifulSoup(r_form.text, 'html.parser')
print('skyx options:', len(soup.select('select[name=skyx] option')))
print('xq options:', len(soup.select('select[name=xq] option')))
print('jxl options:', len(soup.select('select[name=jxl] option')))
